...
<head>
<title>酒店客房管理系统-商品类型管理界面</title>
<%@ include file="common/CommonAdminHead.jsp"%>
</head>
<body
	onload="MM_preloadImages('image/bg_sub_sidebar_bottom_she_副本.jpg','image/splbgl1.jpg','image/spxxgl1.jpg','image/kflxgl1.jpg','image/kfxxgl1.jpg','image/lcgl1.jpg','image/yhgl1.jpg','image/rzgl1.jpg')">
	<!-- header -->
					...
					<div id="search">
						...
						<form id="form1" class="pull-left" name="form1" method="post"
							...
							<label for="Submit"></label> <label for="label5"></label> <label
								...
								class="message">商品类别关键字：</span></label> <label for="textfield"></label>
	...
	<% List goodtyperesult = (List)request.getAttribute("goodtyperesult");
	 ...
	 %>	
	 	 	<tr>
				...
				<th width="70"><a
									href="<s:url action="update"><s:param name="id"><%=goodtyperesults.getTypecode()%></s:param></s:url>" />修改</a></th>
								<th width="70"><a
									href="<s:url action="delete"><s:param name="id"><%=goodtyperesults.getTypecode()%></s:param></s:url>" />删除</a></th>
							</tr>
	 		</tr>
	  <%}
	 		}
